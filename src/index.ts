...
export async function install(version: number = 8, options: any = {}) {
  ...
  return tmpFetch(url)
    ...
    .then((json: { binaries: { [x: string]: string }[] }) =>
      downloadAll(tmpdir, json.binaries[0]["binary_link"])
    )
    .then(verify)
    .then(move)
    .then(extract);
...
function genChecksum(file: any) {
  return new Promise((resolve, reject) => {
    fs.readFile(file, (err: any, data: any) => {
      ...
      resolve(tmpCrypto.createHash("sha256").update(data).digest("hex"));
    });
  });
}
  ...
  return new Promise((resolve, reject) => {
    yauzl.open(
      ...
      (
        ...
        zipFile: {
          readEntry: () => void;
          on: (arg0: string, arg1: (entry: any) => void) => void;
          openReadStream: (
            arg0: any,
            arg1: (err: any, readStream: any) => void
          ) => void;
